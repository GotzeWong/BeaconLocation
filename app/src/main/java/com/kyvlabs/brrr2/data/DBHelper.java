...
public class DBHelper extends SQLiteOpenHelper {
    ...
    public Observable<DBBeacon> getBeaconsByIds(Collection<BeaconIds> beaconCollection, final String auth) {
        ...
        Log.d(LOG_TAG, "getBeaconsByIds before;" + beaconCollection.size() + " beacons");
        Observable<DBBeacon> beaconsToFind = Observable.from(beaconCollection)
        ...
        Observable<DBBeacon> beaconsFromInternet = beaconsToFind
                .filter(new Func1<DBBeacon, Boolean>() {
                    @Override
                    public Boolean call(DBBeacon beacon) {
                        Log.d("GETBEACONS", "NextUpdateTime:" + beacon.getNextUpdateTime() + ":current:" + System.currentTimeMillis() + ": online=" + networkHelper.isOnline());
                        return ((beacon.getNextUpdateTime() < System.currentTimeMillis()) && networkHelper.isOnline());
                    }
                })
                ...
                .flatMap(new Func1<List<BeaconIds>, Observable<BeaconModel>>() {
                    ...
                    public Observable<BeaconModel> call(List<BeaconIds> beaconIdses) {
                        if (beaconIdses.size() > 0) {
                            Log.d(LOG_TAG, "beaconIdses.size() > 0");
                            return networkHelper.getBeacons(beaconIdses, auth);
                        } else {

                            Log.d(LOG_TAG, "Observable.empty");
        ...
        Observable<DBBeacon> beaconsFromLocalStorage = beaconsToFind
                .filter(new Func1<DBBeacon, Boolean>() {
                    @Override
                    public Boolean call(DBBeacon beacon) {
                        return (!(beacon.getNextUpdateTime() < System.currentTimeMillis()) && networkHelper.isOnline());
        ...
        return beaconsFromLocalStorage.mergeWith(beaconsFromInternet);
    }
    ...
    private DBBeacon getBeaconByIds(BeaconIds ids) {
        ...
        beacon.setRssi(ids.getRssi());
        Log.d("DBHelper", "getBeaconByIds:" + beacon.toString());
